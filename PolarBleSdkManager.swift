...
class PolarBleSdkManager : ObservableObject {
    ...
    //private static var deviceId = "C015D22B"
    var deviceId = "C015D22B"
    
    @Published var isBluetoothOn: Bool
    @Published var isBroadcastListenOn: Bool = false
    
    //@Published var deviceConnectionState: DeviceConnectionState = DeviceConnectionState.disconnected(self.deviceId)
    ...
    init() {
        ...
        self.deviceConnectionState = DeviceConnectionState.disconnected(self.deviceId)
    ...
    deinit{
        print("\(self.deviceConnectionState)"+"in deinit"+"student name:"+name!)
    }
    ...
    //when you choose a device and connects to it
    func updateSelectedDevice( deviceId : String) {
        ...
        if case .connected = deviceConnectionState {
            Task { @MainActor in
                self.deviceConnectionState = DeviceConnectionState.connected(deviceId)
            }
        }
    }
    ...
    func connectToDevice() {
        if case .disconnected(let deviceId) = deviceConnectionState {
            do {
                ...
                self.deviceConnectionState=DeviceConnectionState.connected(deviceId)
                //print("now state is \(deviceConnectionState)")
            } catch let err {
                NSLog("Failed to connect to \(self.deviceId). Reason \(err)")
            }
        }
    }
    ...
    func disconnectFromDevice() {
        if case .connected(let deviceId) = deviceConnectionState {
            do {
                ...
                self.deviceConnectionState=DeviceConnectionState.disconnected(deviceId)
                print("\(deviceId)"+"------------------------------- i am hereeeeeeeeeeeeeeeeeeeeeeeeee")
    ...
    var acclist=[String:String]()
    
    func accStreamStart(settings: PolarBleSdk.PolarSensorSetting) {
        if case .connected(let deviceId) = deviceConnectionState {
            let ref = Database.database().reference().child(self.name!).child("acc "+"\(currentTime)")
            Task { @MainActor in
                ...
                .subscribe{ e in
                    ...
                    case .next(let data):
                        ...
                        for item in data.samples {
                            ...
                            self.acclist["\(self.time())"]="x: \(item.x) y: \(item.y) z: \(item.z)"
                            do{
                                let jsonData = try JSONSerialization.data(withJSONObject: self.acclist)
    ...
    var ppglist = [String:String]()
    func ppgStreamStart(settings: PolarBleSdk.PolarSensorSetting) {
        if case .connected(let deviceId) = deviceConnectionState {
            let ref = Database.database().reference().child(self.name!+"(ppg)").child("\(currentTime)")
            Task { @MainActor in
                ...
                .subscribe{ e in
                    ...
                    case .next(let data):
                        if(data.type == PpgDataType.ppg3_ambient1) {
                            ...
                            self.ppglist["\(self.time())"]="\(item.channelSamples[0])"
                                
                            }
                            do{
                                let jsonData = try JSONSerialization.data(withJSONObject: self.ppglist)
                                let jsonObject = try JSONSerialization.jsonObject(with: jsonData)
    ...
    var ppilist=[String:String]()
    
    func ppiStreamStart() {
        if case .connected(let deviceId) = deviceConnectionState {
            let ref = Database.database().reference().child(self.name!+"(ppi)").child("\(currentTime)")
...
//                })
                    ...
                    case .next(let data):
                        if let fileHandle = logFile?.fileHandle {
                            ...
                            for item in data.samples {
                               ...
                               // self.ppilist.append(["\(self.time())":"\(item.ppInMs)"])
                                self.ppilist["\(self.time())"]="\(item.ppInMs)"
                                do{
                                    let jsonData = try JSONSerialization.data(withJSONObject: self.ppilist)
                                    let jsonObject = try JSONSerialization.jsonObject(with: jsonData)
    ...
    var hrlist=[String:String]()
    
    func hrStreamStart() {
        if case .connected(let deviceId) = deviceConnectionState {
            
            let ref = Database.database().reference().child(self.name!+"(hr)").child("\(currentTime)")
            ...
            onlineStreamingDisposables[.hr] = api.startHrStreaming(deviceId)
                ...
                .subscribe{ e in
                    ...
                    case .next(let data):
                        ...
                        self.hrlist["\(self.time())"]="\(data[0].hr)"
                        
                        do{
                            let jsonData = try JSONSerialization.data(withJSONObject: self.hrlist)
                            let jsonObject = try JSONSerialization.jsonObject(with: jsonData)
